...
def convert_tokens(eval_file, qa_id, pp1, pp2):
    ...
    for qid, p1, p2 in zip(qa_id, pp1, pp2):
        ...
        answer_idx_dict[str(qid)] = (p1, p2) 

    return answer_dict, remapped_dict, answer_idx_dict

def evaluate(eval_file, answer_dict):
    f1 = exact_match = total = 0
    for key, value in answer_dict.items():
        if eval_file[key]["answers"][0] == '':  # count类型的，不需要统计
            continue
        total += 1
        ground_truths = eval_file[key]["answers"]
        prediction = value
        # print(prediction, '---', ground_truths)
        # if prediction not in ground_truths:
        #     print(eval_file[key])
...
def test(model, dataset, eval_file, test_i):
    ...
    with torch.no_grad():
        for i, (Cwid, Ccid, Qwid, Qcid, y1, y2, ids) in enumerate(dataset):
            ...
            answer_dict_, _, answer_idx_dict_ = convert_tokens(eval_file, ids.tolist(), ymin.tolist(), ymax.tolist())
            answer_dict.update(answer_dict_)
            answer_idx_dict.update(answer_idx_dict_)
            print("\rSTEP {:8d}/{} loss {:8f}".format(i + 1, len(dataset), loss.item()), end='')
            if((i+1) == num_batches):
                break
    loss = np.mean(losses)
    metrics = evaluate(eval_file, answer_dict)
    f = open("log/answers_idx.json", "w")
    json.dump(answer_idx_dict, f)
    f.close()
